  ...
  def __call__(self,image,mask):
    if random.random() < self.probability:
      ...
      if type(args) is tuple:
        transformed_image = self.functional_transform(
          image,*args,**self.functional_keywords)
        transformed_mask = self.functional_transform(
          mask,*args,**self.functional_keywords)
      else:
        transformed_image = self.functional_transform(
          image,args,**self.functional_keywords)
        transformed_mask = self.functional_transform(
